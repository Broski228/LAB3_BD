...
-- Completed: all tasks (Transaction proc, Views, Indexes, Batch proc, Tests, EXPLAIN instructions)
-- Author: NURLAN ALINUR - 24B031073


-- 0. Preparation: Enable extensions

CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
CREATE EXTENSION IF NOT EXISTS pgcrypto;  --to generate random values
-- GIN on jsonb works without extensions
...
-- 2. Audit trigger: logging INSERT/UPDATE/DELETE
...
-- Apply the trigger to the main tables:
...
-- 3. Test data: customers (>= 10), accounts (>= 10), exchange_rates, transactions
...
-- I'll add a few more accounts for customers
...
-- Get the current rate from->to (latest valid rate where now() BETWEEN valid_from and valid_to OR valid_to IS NULL)
...
-- Function for calculating the amount of a client's transactions today in KZT
...
-- 5. Procedure process_transfer

-- Requirements: ACID, SELECT FOR UPDATE, SAVEPOINTs, Detailed error codes and logging of attempts in audit_log
